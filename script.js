...
function createDynamicMap(mapId, inputId, suggestionsId, coordsCallback, updateInputCallback) {
  // México DF inicial
  const map = L.map(mapId).setView([19.432608, -99.133209], 13);
  ...
  input.addEventListener('input', function() {
    ...
    timeoutAC = setTimeout(() => {
      fetch(`https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=7&q=${encodeURIComponent(query)}`)
        .then(res => res.json())
        .then(results => {
          suggBox.innerHTML = "";
...
// Inicializar mapas y autocompletado
